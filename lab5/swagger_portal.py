...
def generate_openapi(app, title="Hotel API (Lab5)", version="1.0.0",
                     exclude_prefixes=("/static", "/apidocs", "/openapi.json")):
    """
    Автогенерація OpenAPI 3.0 зі всіх Flask-маршрутів.
    POST/PUT/PATCH/DELETE позначені як такі, що потребують basicAuth.
    """
    paths = {}
    for rule in app.url_map.iter_rules():
        if rule.endpoint == "static":
    ...
    return {
        ...
        "info": {
            ...
            "description": (
                "Автогенерація зі всіх Flask маршрутів.\n"
                "GET відкриті, а POST/PUT/PATCH/DELETE позначені як такі, що потребують basicAuth.\n"
                "Auth вводиться у Swagger UI через кнопку Authorize."
            )
        },
        "servers": [{"url": "/"}],
        "paths": paths,
...
def init_swagger(app, title="Hotel API (Lab5)", version="1.0.0"):
    """
    Підключає:
      - /openapi.json (автоген)
      - /apidocs (Swagger UI) з persistAuthorization=False
    Авторизація вводиться користувачем у попапі Swagger UI ("Authorize").
    """
    @app.get("/openapi.json")
    def openapi_json():
    ...
    swaggerui_blueprint = get_swaggerui_blueprint(
        ...
        config={
            "app_name": title,
            # 🟢 головне — не зберігати авторизацію між оновленнями
