...
class DiscountCardDAO:
    ...
    def update_discount_card(card_id, name, discount):
        try:
            cursor = current_app.mysql.connection.cursor()
            cursor.execute("UPDATE discount_cards SET name=%s, discount=%s WHERE card_id=%s", (name, discount, card_id))
            current_app.mysql.connection.commit()
            cursor.close()
        except Exception as e:
    ...
    def delete_discount_card(card_id):
        try:
            cursor = current_app.mysql.connection.cursor()
            cursor.execute("DELETE FROM discount_cards WHERE card_id=%s", (card_id,))
            current_app.mysql.connection.commit()
            cursor.close()
        except Exception as e:
